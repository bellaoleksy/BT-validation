...
cyano1_covariates <- ggplot(data_myxo_full, aes(y=sqrt_BT_cyano_perc,x=sqrt_UPLC_myxo_perc, color=investigator)) +
  ...
  ggtitle("Cyanobacteria")+
  expand_limits(y=1.0,x=0.4)+
  scale_y_continuous(breaks=pretty_breaks(n=5), labels= scales::percent, name = expression(sqrt~cyano["BT"]~"%")) +
  scale_x_continuous(breaks=pretty_breaks(n=5), labels= scales::percent, name = expression(sqrt~myxo["UPLC"]~"%"))+
  scale_color_manual(values=investigator_colors,
                    name="Sample site",
                    breaks=c("Chris","Bella","Whit","Tristan"),
                    labels=c("SRL", "LV", "PR", "RMNP"))+
...
all_stacked_ancova <- ggarrange(chla_panel_ancova,
          ...
          nrow=2,
          widths = c(4,4), heights = c(0.3,1.0))
ggsave("figures/ANCOVA_combined_12x16.tiff", width=12, height=16,units="in")
ggsave("figures/ANCOVA_combined_10x16.tiff", width=10, height=16,units="in")
